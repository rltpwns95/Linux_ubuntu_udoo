...
static long ledkey_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	size = _IOC_SIZE( cmd );//구조체 변수 132
	if( size )
	{
		err = 0;
		if( _IOC_DIR( cmd ) & _IOC_READ ) //10 01
			err = access_ok( VERIFY_WRITE, (void *) arg, size );
		//메모리 접근 가능한지(VERIFY_WRITE) 체크, arg번지부터 size까지
		if( _IOC_DIR( cmd ) & _IOC_WRITE )
			err = access_ok( VERIFY_READ , (void *) arg, size );
		//읽을 수 있는 메모리 공간인지 체크
		if( !err ) return err; //err가 0이면 false
	}
	switch( cmd )
	{
		char buf;
		...
		case IOCTLTEST_GETSTATE :
			...
			return buf;
		case IOCTLTEST_READ :
  			key_read(&(ctrl_info.buff[0]));//변수의 주소를 넘김byte  char*
			if(ctrl_info.buff[0] != 0) //key가 눌리면
				ctrl_info.size=1; //1byte를 리턴
		...
		case IOCTLTEST_WRITE_READ :
			...
			if(ctrl_info.buff[0] != 0) //info.size = 1
				ctrl_info.size=1;
			else
				ctrl_info.size=0;
