...
//		wait_event_interruptible(WaitQueue_Read,sw_no);
		...
		// 100 *100 = 1sec
		//결과는 주석 부분과 동일하다. sw_no가 0일 때 슬립
		//sw_no가 0이 아니면 깨우겠다.
	}
	ret=copy_to_user(buf,&sw_no,count); //sw_no를 사용자쪽에 복사
	sw_no = 0;
	if(ret < 0)
		return -ENOMEM;
    return count;
...
irqreturn_t sw_isr(int irq, void *unuse)  //커널부분, 디바이스x
{
	int i;
	for(i=0;i<ARRAY_SIZE(key);i++)
	{
...
int ledkeydev_init(void)
{
	...
	for(i=0;i<ARRAY_SIZE(key);i++)
	{
		result = request_irq(sw_irq[i], sw_isr, IRQF_TRIGGER_RISING, sw_name[i], NULL);		//디바이스
		if(result)
		{
