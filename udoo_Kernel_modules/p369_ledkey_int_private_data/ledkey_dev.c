...
//		ret = gpio_request(key[i], "gpio key");
		sw_irq[i] = gpio_to_irq(key[i]); //irq넘버 저장
		if(ret<0){
			printk("#### FAILED Request gpio %d. error : %d \n", key[i], ret);
			break;
		} 
...
static void ledkey_free(struct file *filp)
{
	...
	for (i = 0; i < ARRAY_SIZE(key); i++){
//		gpio_free(key[i]);
		free_irq(sw_irq[i],filp->private_data);	//해제
	}
}
...
//	char kbuf;
	int ret;
	char *pSw_no = (char *)filp->private_data; //__번지
#if DEBUG
...
//	ret=copy_to_user(buf,&sw_no,count);
	...
	*pSw_no = 0;
	//다시 앱을 실행했을 때 8값을 가지고 있어서 바로 종료를 막기 위해
	if(ret < 0)
		return -ENOMEM;
    return count;
}
...
int ledkeydev_release (struct inode *inode, struct file *filp)
{
	...
	ledkey_free(filp);
	if(filp->private_data)	//널포인트가 아니면 실행
...
irqreturn_t sw_isr(int irq, void *private_data)  //커널
{
	int i;
...
{
	int i;
	char * sw_name[8]={"key1","key2","key3","key4","key5","key6","key7","key8"};  //주소값 저장
	...
	for(i=0;i<ARRAY_SIZE(key);i++)
	{
		result = request_irq(sw_irq[i], sw_isr, IRQF_TRIGGER_RISING, sw_name[i], filp->private_data);		//디바이스 filp->private_data = void *
		if(result)
		{
